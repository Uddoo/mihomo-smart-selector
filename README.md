...
| **当前网络能访问哪些常用服务？** | 独立的[连通性测试](docs/connectivity.md)页面按地区展示 48 个服务，由浏览器本机采样，支持全部重测与分组刷新。 |
...
| **观察长期表现** | 启用监控计划并保持后端服务运行，关闭网页仍会采集历史；查看健康状态、覆盖率与事件，按需开启故障自动切换。 |
...
**持续监控：** 启用监控计划后，本项目后端会定期通过 Mihomo Controller 发起探测，并将结果保存在配置的 SQLite 数据库中。
支持同时监控多个策略组，每组独立配置候选、服务模板、启停状态和故障自动切换；同一节点加入不同任务时，观测历史分别保存。所有任务共享探测预算与存储配额。
无论部署在路由器、Windows 还是 macOS，只要运行本项目的设备保持唤醒、后端进程持续运行，关闭网页都不会中断监控。
退出桌面程序或停止路由器上的本项目服务会停止采样；仅连接到路由器上的 Mihomo，不会把本项目的监控计划交给路由器运行。
