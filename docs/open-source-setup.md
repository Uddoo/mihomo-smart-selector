...
2026-09-07 已将仓库设为 Public，定位为“支持已验证环境、仍在开发中的自托管工具”。
已通过 GitHub API 确认启用私密漏洞报告、Dependabot 依赖告警及安全更新、
密钥扫描和密钥推送保护。CodeQL 已在公开后启动，Go 与 JavaScript/TypeScript
的具体扫描状态和结果以 [Actions](https://github.com/Uddoo/mihomo-smart-selector/actions)
及 Security 页面为准；工作流运行成功不等于没有安全告警。

维护者已确认保留截图中的真实使用信息。当前 `main` 要求验证及 ARM64、AMD64 构建
三项检查，管理员仍可绕过；没有把规则配置存在等同于每次提交都经过强制审核。
二进制 Release 和未验证设备的支持承诺不在本次公开范围内。
