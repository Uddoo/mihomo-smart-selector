...
在主导航打开 **连通性测试**（直达路径 `/#/connectivity`）。页面按服务的主要用途展示 48 个常用服务，点击测试按钮后才从当前浏览器发起请求。它与节点扫描、持续监控分别使用自己的数据。

| 服务类型 | 数量 | 服务示例 |
| --- | --- | --- |
| AI | 6 | DeepSeek、Claude、ChatGPT、AI Studio、PixPix、Mistral AI |
| 社交 | 10 | 微信、QQ、小红书、LINE、Facebook、X |
| 影音 | 7 | 哔哩哔哩、YouTube、Netflix、Spotify、TikTok |
| 开发与云 | 4 | GitHub、Cloudflare、Oracle、npm |
| 搜索资讯 | 9 | 百度、Google、Bing、Wikipedia、BBC |
| 购物 | 6 | 京东、淘宝、Amazon、MercadoLibre |
| 游戏 | 2 | 任天堂、Steam |
| 工具 | 4 | 小米、Sony、Apple、Zoom |

每项服务只归入一个主要用途类别；综合品牌按本面板展示的服务范围归类。PixPix 根据其[官方产品介绍](https://www.pixpix.com/)归入 AI 创作服务。分类不改变探测入口、响应规则或策略组绑定。

## 使用

- 进入页面显示“尚未测试”，点击 **开始测试**或分组/单服务测试按钮后才发送请求；切换 Tab 后保留结果、异常筛选和分组折叠状态，返回时不会自动重测。刷新或关闭整个浏览器页面后回到未测试状态，仍需手动启动。
- 类型按钮筛选服务，搜索支持服务名称、英文名称和 ID，并与“我的服务”“仅看异常”组合。类型数量表示当前全部/我的服务范围内的总数，“显示”数量表示筛选后的结果；折叠分组不改变筛选范围。
- 主测试按钮、**重测异常**与**测试本组**遵循当前分类和搜索范围；主按钮显示“测试 AI”“测试筛选服务”等实际范围。未设置分类或搜索时，**全部重新测试**更新全部服务；详情中的**重新测试此服务**只执行该服务的 8 次采样。筛选没有匹配项时禁用测试并提供清除筛选入口。
...
- 点击类型名称可折叠或展开该组，折叠后仍保留可达数量、平均延迟、不稳定与异常数量。切换分类或搜索不会自动发起测试，也不会取消当前已启动的测试；本轮状态继续显示启动时的范围，仍可停止。
...
- **全部服务 / 我的服务**切换保留本次页面会话中的结果、分类与搜索。我的服务仅包含有效绑定、且在固定清单中有对应卡片的服务；**测试我的服务**对其中每个服务执行 8 次浏览器采样，同一服务绑定多个策略组也只测试一次。分类、搜索、分组测试和异常重测遵循当前视图的范围。
...
- 首次进入、返回页面、切换视图、刷新配置、发现节点变化均不自动开始测试。未绑定时提供扫描工作台入口；配置读取失败时不把旧配置作为当前有效关联，全部服务视图仍可手动执行浏览器探测。配置快照与测试结果一样仅存在当前文档内存中。
