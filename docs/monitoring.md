...
1. 在“持续监控 → 监控设置”选择业务 Selector、服务模板和候选，点击开始监控。每个 Controller 当前支持一个方案；“监控候选上限”可设为 1–30 个，默认 6 个。提高上限后手动勾选新增候选；上限按节点数计算，多目标模板不会减少可选节点数，每个模板支持 1–6 个探测目标。
...
5. 在“监控设置”调整方案、显式开启故障自动切换、修改保留策略或下载诊断包。降低候选上限不会自动删选，超额时须手动取消选择才能保存；刷新后失效的已选节点也须显式移除。上限随方案保存，暂停、继续和重启均保留；只改上限不会重建历史序列。监控内部切换页签会保留未保存的表单；离开整个监控页面不会自动保存草稿。暂停监控同时暂停自动切换。
...
- 后台单 worker 与手动扫描共用探测槽位，后台滚动一分钟请求预算为 `max(12, 已选节点数 × 模板探测目标数 × 2)`，按实际选择计算。基准、当前节点检查、确认、手动复测与切换前验证共享预算；保存方案不会清空已消耗的额度。单目标 6 节点仍为 12 次/分钟，30 节点为 60 次/分钟；最多 30 节点 × 6 目标时为 360 次/分钟。
