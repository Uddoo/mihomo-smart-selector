...
以下四张中文界面图通过 Microsoft Edge 从部署版本 `fe393a8` 实际截取。节点名称、成功率、时延、事件和开关状态属于当时的运行快照，不是内置演示数据、默认配置或跨网络性能基准。采集时只浏览页面，没有暂停监控、执行复测或修改设置。
...
![中文监控概览：当前选择、异常提示和 24 小时 HTTPS 健康排名](assets/screenshots/monitoring-overview-live-20260911-zh-CN.jpg)

- “策略组当前选择”与健康徽章反映当前选择和近期状态；点击“查看当前节点趋势”进入对应序列。
- “需要关注”中的节点可直接查看趋势。“0 个缺测”是当前状态摘要，不表示整个历史窗口没有缺测。
- 排名行同时展示当前状态、历史成功率、P95、失败段与覆盖率。图中 `JP4-HY2` 当前为健康，但历史成功率为 75.4%、健康分为 59.4；近期恢复不抹去窗口内的失败。
...
![中文节点详情：观测序列、评分依据、P50/P95、事件标记与健康区间](assets/screenshots/monitoring-node-history-live-20260911-zh-CN.jpg)
...
图中时间轴截至 `2026/9/11 11:49:14`，这是该趋势快照的时点。底部最多展示 12 个相关事件按钮与标记；完整记录应进入事件时间线查看。红色小时色块表示该小时存在失败，不表示整小时宕机；折线空白也不能当作零延迟。

### 事件时间线：区分没有提示和没有故障

![中文事件时间线：Provider 共同异常提示、已加载事件筛选和趋势定位](assets/screenshots/monitoring-events-live-20260911-zh-CN.jpg)
...
![中文监控设置：已启用的故障自动切换、方案编辑、存储保留与诊断导出入口](assets/screenshots/monitoring-settings-live-20260911-zh-CN.jpg)
