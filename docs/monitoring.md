...
概览默认及 `view=full` 继续返回完整记录。前端使用
`GET /monitor/tasks/{task_id}/overview?window=24h&view=summary` 读取汇总，响应不包含各行的 `series`。
进入节点详情时增加 `series_id`，仅返回当前任务中该节点的最近采样；其他行仍省略明细。
响应的 `sample_series_id` 标识本次请求的明细目标，前端用它和窗口、请求代次隔离旧响应。
历史或其他任务的序列不会因此泄露明细，历史趋势仍通过原有任务级 timeline 接口读取。
未指定任务的旧概览也支持相同视图参数，但多任务时继续返回 HTTP 409。
