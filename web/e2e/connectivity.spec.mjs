...
test('product categories and search scope explicit tests without probing on filter changes', async ({page}) => {
  ...
  await expect(page.getByRole('button',{name:'按地区',exact:true})).toHaveCount(0)
  await expect(page.locator('.connectivity-group')).toHaveCount(8)
  await types.getByRole('button',{name:'AI 6',exact:true}).click()
  await expect(page.locator('.service-card')).toHaveCount(6)
  for (const id of ['deepseek','claude','chatgpt','aistudio','mistral','pixpix']) await expect(page.locator(`[data-service="${id}"]`)).toBeVisible()
  expect(calls).toHaveLength(0)
  await page.getByRole('button',{name:'测试AI',exact:true}).click()
  await expect(page.locator('.test-all')).toBeEnabled()
  expect(calls).toHaveLength(48)
  expect(calls.every(call=>targets.find(t=>t.id===call.id).category==='ai')).toBe(true)
  const previous=await page.locator('[data-service="chatgpt"]').textContent()
  await types.getByRole('button',{name:'社交 10',exact:true}).click()
  await expect(page.locator('.service-card')).toHaveCount(10)
  await page.getByRole('searchbox',{name:'搜索服务'}).fill('  GPT  ')
  await expect(page.getByText('没有匹配的服务',{exact:true})).toBeVisible()
  await expect(page.locator('.test-all')).toBeDisabled()
  await types.getByRole('button',{name:'全部类型 48',exact:true}).click()
  await expect(page.locator('.service-card')).toHaveCount(1)
  await expect(page.locator('[data-service="chatgpt"]')).toHaveText(previous)
  await page.getByRole('button',{name:'节点目录',exact:true}).click()
  await page.getByRole('button',{name:'连通性测试',exact:true}).click()
  await expect(page.getByRole('searchbox',{name:'搜索服务'})).toHaveValue('  GPT  ')
  expect(calls).toHaveLength(48)
  await page.getByRole('button',{name:'测试筛选服务',exact:true}).click()
  await expect(page.locator('.test-all')).toBeEnabled()
  expect(calls.slice(48).map(call=>call.id)).toEqual(Array(8).fill('chatgpt'))
  await page.getByRole('button',{name:'清空搜索',exact:true}).click()
  await expect(page.getByRole('searchbox',{name:'搜索服务'})).toBeFocused()
  await expect(page.locator('.service-card')).toHaveCount(48)
})

test('changing category during a run preserves its scope and allows cancellation', async ({page}) => {
  ...
  await types.getByRole('button',{name:'影音 7',exact:true}).click()
  await expect(page.locator('.connectivity-progress')).toContainText('正在测试 · AI')
  await expect(page.locator('.connectivity-progress')).toContainText('0/48')
  await expect(page.locator('.test-all')).toBeDisabled()
  await expect(page.locator('.service-latency')).toHaveText(Array(7).fill('未测试'))
  await page.getByRole('button',{name:'停止',exact:true}).click()
  for(const route of held) await route.fulfill({status:204}).catch(()=>{})
  await types.getByRole('button',{name:'AI 6',exact:true}).click()
  await expect(page.locator('.service-latency')).toHaveText(Array(6).fill('已停止'))
  await expect(page.locator('.sample-dot.pending')).toHaveCount(48)
...
test('explicit start tests 48 services with eight samples and no credentials', async ({page}) => {
  ...
  await expect(page.locator('[data-group="development"] .group-statistics')).toContainText('3/4')
  await expect(page.locator('[data-group="development"] .group-statistics')).toContainText('1 项不稳定')
})
...
test('group refresh only replaces that group and starts exactly 32 new requests', async ({page}) => {
  ...
  await allButton(page).click()
  await expect(allButton(page)).toBeEnabled({timeout:15000})
  const before=await page.locator('[data-group="social"]').innerText()
  const count=calls.length
  await page.getByRole('button',{name:'测试开发与云分组'}).click()
  await expect(allButton(page)).toBeEnabled({timeout:15000})
  expect(calls.slice(count)).toHaveLength(32)
  expect(new Set(calls.slice(count).map(call=>call.id))).toEqual(new Set(targets.filter(t=>t.category==='development').map(t=>t.id)))
  expect(await page.locator('[data-group="social"]').innerText()).toBe(before)
})
...
test.describe('touch sample inspection', () => {
  ...
  test('a card opens visible results on tap and closes without losing focus', async ({page}) => {
    ...
    expect(cardSize.height).toBeGreaterThanOrEqual(80)
    expect(cardSize.height).toBeLessThanOrEqual(96)
...
test('completed observations and collapsed groups survive navigation without new probes', async ({page}) => {
  ...
  await page.getByRole('button',{name:'收起开发与云分组'}).click()
  await expect(page.locator('[data-group="development"] .group-statistics')).toContainText('4/4')
  await page.getByRole('button',{name:'节点目录',exact:true}).click()
  await page.getByRole('button',{name:'连通性测试',exact:true}).click()
  ...
  await expect(page.getByRole('button',{name:'展开开发与云分组'})).toHaveAttribute('aria-expanded','false')
  await expect(allButton(page)).toBeEnabled()
  expect(calls).toHaveLength(384)
  await page.getByRole('button',{name:'展开开发与云分组'}).click()
  await expect(page.locator('[data-group="development"] .service-card')).toHaveCount(4)
})
...
test('entering, returning and reloading stay idle until a test action is clicked', async ({page}) => {
  ...
  await page.getByRole('button',{name:'Test Development & cloud group'}).click()
  await expect(allButton(page)).toBeEnabled({timeout:15000})
  expect(calls).toHaveLength(32)
  expect(calls.every(call=>targets.find(target=>target.id===call.id).category==='development')).toBe(true)
  await expect(page.locator('[data-group="social"] .service-latency')).toHaveText(Array(10).fill('Not tested'))
})

test('compact category controls preserve filters across breakpoints and fit themed mobile layouts', async ({page}) => {
  const {calls}=await connectivityFixture(page,{mixed:true})
  await page.addInitScript(()=>{localStorage.setItem('mss-palette','nord');localStorage.setItem('mss-theme','dark')})
  await page.emulateMedia({reducedMotion:'reduce'})
  await page.goto('/#/connectivity')
  await allButton(page).click(); await expect(allButton(page)).toBeEnabled({timeout:15000})
  const folder=new URL('../../.impeccable/review/connectivity-categories/',import.meta.url)
  await fs.mkdir(folder,{recursive:true})
  for (const [width,height,locale,name] of [[390,844,'zh-CN','mobile-nord'],[320,812,'en','mobile-nord-en'],[768,900,'en','tablet-nord-en'],[1440,1000,'zh-CN','desktop-nord']]) {
    await page.setViewportSize({width,height})
    await page.locator('.language-select select').selectOption(locale)
    if(width<=760) {
      const select=page.getByRole('combobox',{name:locale==='en'?'Service type':'服务类型',exact:true})
      await select.focus(); await select.selectOption('ai')
      await expect(page.locator('.category-options')).toBeHidden()
    } else {
      await page.locator('.category-options').getByRole('button',{name:'AI 6',exact:true}).click()
      await expect(page.locator('.mobile-category')).toBeHidden()
    }
    await expect(page.locator('.service-card')).toHaveCount(6)
    ...
    expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true)
    expect(await page.locator('.service-card').evaluateAll(cards=>cards.every(card=>card.scrollWidth<=card.clientWidth))).toBe(true)
    const invalid=await page.locator('.service-card .sample-strip').evaluateAll(strips=>strips.some(strip=>strip.scrollWidth>strip.clientWidth))
