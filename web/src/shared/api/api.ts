...
async function withResponse<T>(
  ...
  const {
    timeoutMs = /^(GET|HEAD)$/i.test(init.method || 'GET') ? 10000 : 25000,
    signal: caller,
    ...request
  } = init
  ...
  const timer = setTimeout(
    () =>
      controller.abort(
        new Error(
          /^(GET|HEAD)$/i.test(init.method || 'GET')
            ? '读取超时，请重试'
...
export function downloadAPI(path: string, body: unknown): Promise<Blob> {
  return withResponse(
    path,
    { method: 'POST', body: JSON.stringify(body), timeoutMs: 30000 },
    async (response) => {
      if (!response.ok) {
        const result = await response.json().catch(() => ({}))
        throw new APIError(result?.error || '下载失败', response.status)
      }
      if (response.status === 204)
        throw new Error('导出响应为空，可能已被外部下载器接管，请检查下载器。')
      const result = await response.json()
      if (result.encoding !== 'base64' || typeof result.data !== 'string' || !result.data)
