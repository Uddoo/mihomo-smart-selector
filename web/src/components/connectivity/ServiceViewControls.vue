...
defineProps<{view: ServiceView; count: number; changed: number; loading: boolean; available: boolean; updatedAt: number | null; locked: boolean}>()
defineEmits<{view: [value: ServiceView]; refresh: []}>()
</script>

<template>
  <div class="service-view-controls">
    <div class="service-view-options" role="group" :aria-label="t('服务视图')">
      <button :aria-pressed="view === 'all'" @click="$emit('view', 'all')">{{ t('全部服务') }}</button>
    ...
    <div class="binding-refresh">
      ...
      <span v-else-if="updatedAt" class="config-read-time">{{ t('配置读取于 {time}', {time: formatDate(updatedAt, 'time')}) }}</span>
...
.service-view-options button[aria-pressed="true"] { color:var(--nav-text); background:var(--selection-bg); border-color:var(--accent); }
.service-view-options button[aria-pressed="true"] span { color:var(--selection-muted); }
...
@media (max-width:760px) {
  .service-view-controls { gap:8px; }
  .service-view-options { gap:6px; }
  .service-view-options button { padding-inline:10px; font-size:13px; }
  .binding-refresh { margin-inline-start:auto; }
  .binding-refresh button { min-height:44px; padding-inline:10px; }
  .config-read-time { display:none; }
}
</style>
