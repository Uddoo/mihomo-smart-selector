...
const props = defineProps<{
  view: 'regions' | 'mine'; busy: boolean; stopped: boolean; finishedAt: number | null; lastScope: string; onlyIssues: boolean;
...
<template>
  <div class="connectivity-toolbar">
    <div class="connectivity-actions">
      <button class="primary test-all" :disabled="busy || !summary.total" @click="$emit('retest')"><RefreshCw :size="16" aria-hidden="true"/>{{ t(view === 'mine' ? '测试我的服务' : progress.total ? '全部重新测试' : '开始测试') }}</button>
  ...
  <div class="connectivity-progress">
    <span><span role="status">{{ statusLabel }} · {{ t(progress.total ? lastScope : view === 'mine' ? '我的服务' : '全部服务') }}</span><span v-if="progress.total"> · {{ progressLabel }}</span></span>
...
.filter-issues[aria-pressed="true"] { color:var(--blue); border-color:var(--blue); background:var(--soft); }
...
.connectivity-progress progress::-webkit-progress-value { background:var(--blue); }
.connectivity-progress progress::-moz-progress-bar { background:var(--blue); }
