...
export function retentionCapacity(candidateCount: number, policy: MonitorRetention) {
  const values = [candidateCount, policy.raw_days, policy.aggregate_days, policy.max_raw_samples, policy.max_hourly]
  if (values.some(value => !Number.isSafeInteger(value) || value <= 0) || candidateCount > 30) return null
  ...
  const rawPerDay = candidateCount * 720 + 2160
