...
import {compareNames, protocolLabel, providerLabel} from './nodeCatalog'
...
const active = computed(() => !!filters.query || !!filters.regions.length || !!filters.providers.length || !!filters.protocols.length)
...
const dimensions = computed(() => [
  {key: 'regions' as const, label: '地区', options: options(node => node.inferred_region || '', code => regionLabel(code || undefined))},
...
function options(value: (node: NodeSummary) => string, label: (value: string) => string) {
  ...
  for (const node of nodes.value) { const key = value(node); counts.set(key, (counts.get(key) || 0) + 1) }
...
function chipLabel(key: string, value: string) {
  return key === 'regions' ? regionLabel(value || undefined) : key === 'providers' ? providerLabel(value) : protocolLabel(value)
}
...
function inherit() { filters.regions = [...state.areas.value]; filters.providers = [...state.providerSet.value]; filters.query = ''; filters.protocols = [] }
...
async function resize() {
  ...
  await nextTick()
  if (selected.value) { if (narrow.value) { if (!drawer.value?.open) drawer.value?.showModal() } else { drawer.value?.close(); detail.value?.focus() } }
}
...
<template>
  <section class="catalog" aria-label="节点目录浏览">
    ...
    <div class="catalog-summary"><span role="status">筛选结果 <b>{{ sorted.length }}</b> 个 · 总计 {{ nodes.length }} 个</span><small class="region-hint">地区为推断结果</small><small class="mobile-table-hint">左右滑动表格查看来源与协议</small></div>
