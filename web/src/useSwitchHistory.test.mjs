...
const pending = {
  id: 41, scan_id: 'scan-41', group: 'Default', previous: 'HK-01', selected: 'JP-01',
  status: 'unknown', audit_persisted: true, reason: 'Readback unavailable', created_at: '2026-09-16T08:00:00Z',
}
...
function deferredReads(t) {
  ...
  t.mock.method(globalThis, 'fetch', (_url, init) => new Promise((resolve, reject) => {
    reads.push({signal: init.signal, resolve: events => resolve(Response.json(events)), reject})
  }))
  return reads
}
...
test('a failed independent refresh retains loaded records and their successful read time', async t => {
  ...
  globalThis.fetch.mock.mockImplementation(async () => Response.json({error: 'History unavailable'}, {status: 503}))
...
test('disposal cancellation prevents late reads from changing existing history or showing an error', async t => {
  ...
  assert.equal(state.historyLoading.value, false)
  const afterDisposal = {
    history: state.history.value, loading: state.historyLoading.value, loaded: state.historyLoaded.value,
    error: state.historyError.value, updatedAt: state.historyUpdatedAt.value,
  }
  reads[0].resolve([pending])
  await staleRead
...
test('discovery cancellation removes the caller listener and settles loading without a false history error', async t => {
  ...
  t.mock.method(globalThis, 'fetch', (_url, init) => new Promise((_, reject) => {
