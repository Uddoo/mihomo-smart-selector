...
<template>
          <aside class="panel compare" :aria-label="t('候选详情')">
            ...
            <template v-if="candidate">
              ...
              <table class="comparison-table" :aria-label="t('当前与候选指标')"><thead><tr><th scope="col">{{ t('指标') }}</th><th scope="col">{{ t('当前') }}</th><th scope="col">{{ t('候选') }}</th></tr></thead><tbody><tr><th scope="row">{{ t('P95 时延') }}</th><td>{{ currentResult ? latency(currentResult.p95_ms) : t('本次未测') }}</td><td>{{ latency(candidate.p95_ms) }}</td></tr><tr><th scope="row">{{ t('成功率') }}</th><td>{{ currentResult ? percentLabel(currentResult.success_rate) : t('本次未测') }}</td><td>{{ percentLabel(candidate.success_rate) }}</td></tr></tbody></table>
