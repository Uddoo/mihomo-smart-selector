...
import type { Group, NodeSummary } from '../../shared/types/models'
...
export function useNodeCatalog(
  ...
  const sourceNodes = computed(() => {
    ...
    const members = new Set(
      groups.value.find((group) => group.name === groupScope.value)?.all || [],
    )
    return nodes.value.filter((node) => members.has(node.name))
  })
  const sorted = computed(() =>
    sortCatalog(
      filterCatalog(sourceNodes.value, filters, regionLabel),
      sort.value,
      descending.value,
      regionLabel,
    ),
  ...
  function clear() {
    ...
    filters.status = ''
    groupScope.value = null
  }
  return { filters, sort, descending, pageSize, page, sorted, clear, groupScope, sourceNodes }
}

export type NodeCatalog = ReturnType<typeof useNodeCatalog>
export interface NodeCatalogPageState {
  nodes: Ref<NodeSummary[]>
  regionLabel: (code?: string) => string
  catalog: NodeCatalog
  areas: Readonly<Ref<string[]>>
  providerSet: Readonly<Ref<string[]>>
