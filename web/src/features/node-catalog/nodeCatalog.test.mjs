...
const nodes = [
  {
    name: 'JP10-HY2',
    provider: 'Provider_B26FA7',
    protocol: 'Hysteria2',
    inferred_region: 'JP',
    region_source: 'name-inferred',
  },
  {
    name: 'JP2-HY2',
    provider: 'second',
...
test('dimensions intersect and values within each dimension form a union, including missing metadata', () => {
  assert.deepEqual(
    filterCatalog(
      nodes,
      filters({ regions: ['JP', 'US'], providers: ['second'], protocols: ['hysteria2'] }),
      labels,
    ).map((n) => n.name),
    ['JP2-HY2'],
  )
  assert.deepEqual(
    filterCatalog(nodes, filters({ regions: [''], providers: [''], protocols: [''] }), labels).map(
      (n) => n.name,
    ),
    ['mystery'],
  )
...
test('natural ordering and descending sort preserve original discovery order and data', () => {
  ...
  assert.deepEqual(
    sortCatalog(nodes.slice(0, 2), 'name', false, labels).map((n) => n.name),
    ['JP2-HY2', 'JP10-HY2'],
  )
  assert.deepEqual(
    sortCatalog(nodes.slice(0, 2), 'name', true, labels).map((n) => n.name),
    ['JP10-HY2', 'JP2-HY2'],
  )
...
test('special entries are hidden by default but recoverable, while dynamic and ambiguous proxies remain', () => {
  const input = [
    ...
    { name: '自动线路', entry_kind: 'dynamic', region_source: 'dynamic' },
    {
      name: 'JP via HK',
      entry_kind: 'proxy',
      region_source: 'ambiguous',
      region_candidates: ['JP', 'HK'],
      region_evidence: ['JP', 'HK'],
    },
  ...
  assert.equal(
    filterCatalog(input, filters({ status: 'ambiguous', query: '日本' }), labels)[0].name,
    'JP via HK',
  )
  ...
  assert.equal(
    regionStatus({ ...input[2], inferred_region: 'JP', region_source: 'manual' }),
    'identified',
  )
})
