...
export interface MonitorPlanEditorProps {
  ...
  disabled: boolean
  retentionPolicy?: MonitorRetention | null
}

export function useMonitorPlanEditor(
  props: MonitorPlanEditorProps,
  events: {
    saved: (plan: MonitorPlan) => void
    busy: (value: boolean) => void
    draft: (draft: MonitorDraft) => void
  },
) {
  ...
  const group = ref(props.draft?.group ?? props.plan?.group ?? ''),
    profile = ref(props.draft?.profile ?? props.plan?.profile_id ?? '')
  const chosen = ref([...(props.draft?.chosen ?? props.plan?.nodes.map((n) => n.name) ?? [])])
  const candidateLimit = ref<number | string>(
  ...
  const blockedGroups = computed(() =>
    props.tasks
      .filter((task) => task.plan.task_id !== props.plan?.task_id)
      .map((task) => task.plan.group),
  )
  const workload = computed(() =>
    monitorWorkload(props.tasks, props.services, {
      taskID: props.plan?.task_id || '',
      draft: draft.value,
    }),
  )
  ...
  const catalogBusy = ref(false),
    ...
    saveError = ref(''),
    saving = ref(false)
  const storedRetention = ref<MonitorRetention | null>(null),
    retentionError = ref(false)
  const retention = computed(() => props.retentionPolicy || storedRetention.value)
  const profiles = computed(
    () => props.services?.profiles.filter((p) => !p.requires_configuration) || [],
  )
  const filtered = computed(
    () =>
      catalog.value?.nodes.filter((n) =>
        n.name.toLowerCase().includes(query.value.toLowerCase()),
      ) || [],
  )
  const missing = computed(() =>
    catalog.value
      ? chosen.value.filter((name) => !catalog.value!.nodes.some((n) => n.name === name))
      : [],
  )
  const maxLimit = computed(() => catalog.value?.limits.max_candidate_limit || 30)
  const validLimit = computed(
    () =>
      typeof candidateLimit.value === 'number' &&
      Number.isInteger(candidateLimit.value) &&
      candidateLimit.value >= 1 &&
      candidateLimit.value <= maxLimit.value,
  )
  const overLimit = computed(
    () => validLimit.value && chosen.value.length > Number(candidateLimit.value),
  )
  const validProfile = computed(
    () =>
      !!catalog.value &&
      catalog.value.probe_count >= 1 &&
      catalog.value.probe_count <= catalog.value.limits.max_probe_count,
  )
  const estimated = computed(
    () => (chosen.value.length * 720 + 2160) * (catalog.value?.probe_count || 1),
  )
  const budget = computed(() => {
    const c = catalog.value
    return c
      ? Math.max(
          c.limits.min_requests_per_minute,
          chosen.value.length * c.probe_count * c.limits.requests_per_candidate_probe,
        )
      : 0
  })
  ...
  async function loadRetention() {
    ...
    try {
      ...
      if (!disposed && !controller.signal.aborted) retentionError.value = true
    }
  }
  ...
  function defaults() {
    ...
    if (!group.value)
      group.value = available.find((g) => /ChatGPT/i.test(g.name))?.name || available[0]?.name || ''
    if (!profile.value && group.value)
      profile.value =
        props.services?.suggestions[group.value] ||
        props.services?.default_profile_id ||
        profiles.value[0]?.id ||
        ''
  }
  watch(() => [props.groups, props.services], defaults)
  watch([group, profile], ([nextGroup], [oldGroup]) => {
    if (!initializing) void loadCatalog(nextGroup === oldGroup)
  })
  async function loadCatalog(preserve = false) {
    ...
    catalog.value = null
    catalogError.value = ''
    if (!group.value || !profile.value) {
      catalogBusy.value = false
      return
    }
    catalogBusy.value = true
    try {
      const result = await api<MonitorCatalog>(
        `/monitor/catalog?group=${encodeURIComponent(group.value)}&profile_id=${encodeURIComponent(profile.value)}`,
      ...
      if (!preserve)
        chosen.value = result.suggested.slice(
          0,
          validLimit.value ? Number(candidateLimit.value) : result.limits.default_candidate_limit,
        )
    } catch (e) {
      if (!disposed && rev === catalogRevision)
        catalogError.value = e instanceof Error ? e.message : '无法读取候选'
    } finally {
      if (rev === catalogRevision) catalogBusy.value = false
    }
  }
  ...
  async function save() {
    ...
    saveError.value = ''
    try {
      const result = await api<MonitorPlan>(
        props.plan ? taskPath(props.plan.task_id) : '/monitor/tasks',
        {
      ...
      if (!disposed) saveError.value = e instanceof Error ? e.message : '保存失败'
    } finally {
      saving.value = false
      events.busy(false)
    }
