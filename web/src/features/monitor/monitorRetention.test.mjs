...
test('recommendations never reduce existing caps or exceed server limits', () => {
  assert.deepEqual(
    retentionCapacity(6, { ...defaults, max_raw_samples: 800000, max_hourly: 80000 })
      .recommendation,
  ...
  assert.equal(
    retentionCapacity(30, { ...defaults, aggregate_days: 130 }).recommendation.max_hourly,
    100000,
  )
})

test('empty and invalid drafts do not produce NaN, Infinity or fictitious retention', () => {
  for (const count of [0, -1, 31, 1.5, NaN, Infinity])
    assert.equal(retentionCapacity(count, defaults), null)
  for (const key of ['raw_days', 'aggregate_days', 'max_raw_samples', 'max_hourly']) {
    for (const value of ['', 0, -1, 1.5, NaN, Infinity])
      assert.equal(retentionCapacity(12, { ...defaults, [key]: value }), null)
  }
})
...
test('drafts outside server policy ranges do not offer unusable adjustments', () => {
  for (const change of [
    ...
    { aggregate_days: 366 },
    { aggregate_days: 6 },
    { raw_days: 8, aggregate_days: 7 },
    { max_raw_samples: 999 },
    { max_raw_samples: 1000001 },
    { max_hourly: 999 },
    { max_hourly: 100001 },
  ]) {
