...
export function useMonitorTasks() {
  const tasks = shallowRef<MonitorTask[]>([]),
    scheduler = shallowRef<MonitorScheduler | null>(null),
    retention = shallowRef<MonitorRetention | null>(null)
  const selected = shallowRef(''),
    failure = shallowRef(''),
    capacityFailure = shallowRef(''),
  ...
  let policyTimer: ReturnType<typeof setTimeout> | undefined,
    policyController: AbortController | undefined,
    policyGeneration = 0
  try {
    selected.value = localStorage.getItem('mss.monitor.task') || ''
  } catch {
  ...
  function choose(id: string) {
    ...
    if (id !== 'new') {
      try {
        localStorage.setItem('mss.monitor.task', id)
      } catch {
        /* optional */
      }
    }
  ...
  async function refresh() {
    ...
    controller = request
    try {
      const snapshot = await api<{ tasks: MonitorTask[]; scheduler: MonitorScheduler }>(
        '/monitor/tasks?include=scheduler',
      ...
      if (
        selected.value !== 'new' &&
        !tasks.value.some((task) => task.plan.task_id === selected.value)
      )
        choose(tasks.value[0]?.plan.task_id || 'new')
  ...
  function updateRetention(policy: MonitorRetention) {
    ...
    capacityFailure.value = ''
    if (!disposed && !document.hidden && navigator.onLine)
      policyTimer = setTimeout(() => void readPolicy(), 60000)
  }
  async function readPolicy() {
    ...
    try {
      const policy = await api<MonitorRetention>('/monitor/retention', {
      ...
      if (!disposed && read === policyGeneration) {
        retention.value = null
        capacityFailure.value = '共享容量读取失败，暂不能确认预算和保留时间。'
      }
    } finally {
      if (!disposed && read === policyGeneration)
        policyTimer = setTimeout(() => void readPolicy(), retention.value ? 60000 : 15000)
    }
  }
  function saved(plan: MonitorPlan) {
    ...
    tasks.value = previous
      ? tasks.value.map((task) => (task.plan.task_id === plan.task_id ? updated : task))
      : [...tasks.value, updated]
    choose(plan.task_id)
    void refresh()
  }
