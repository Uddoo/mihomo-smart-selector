...
const dirty = computed(() =>
  tasks.value
    .filter((task) => draftChanged(states.value[task.plan.task_id]?.draft, task.plan))
    .map((task) => task.plan.task_id),
)
const canCreate = computed(() =>
  props.groups.some((group) => !tasks.value.some((task) => task.plan.group === group.name)),
)
...
function saveState(id: string, value: MonitorViewState) {
  states.value[id] = value
}
function saveNewDraft(value: MonitorDraft) {
  saveState('new', { ...(states.value.new || emptyView()), draft: value })
}
function created(plan: MonitorPlan) {
  delete states.value.new
  saved(plan)
}
...
<template>
  <div class="monitor-hub">
    <div
      ...
      class="notice error"
      role="alert"
      >{{ translateMessage(failure) }} <button @click="refresh">{{ t('重新读取') }}</button></div
    >
    <p
      v-if="!initialized && !failure"
      role="status"
      >{{ t('正在读取监控策略组…') }}</p
    >
    <template v-if="initialized">
      ...
      <MonitorTaskView
        ...
        :retention="retention"
        @retention-read="updateRetention"
        :total-candidates="workload.candidates"
        :total-tasks="workload.groups"
        @view-state="saveState"
        @busy="busy = $event"
        @saved="saved"
