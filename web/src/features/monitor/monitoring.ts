...
export interface MonitorCatalog {
  ...
  limits: {
    ...
    min_requests_per_minute: number
    requests_per_candidate_probe: number
  }
}
...
export function monitorTime(value?: string) {
  return !value || value.startsWith('0001-') ? t('尚无记录') : formatDate(new Date(value))
}
export function monitorPercent(value: number) {
...
export interface MonitorTimeline {
  ...
  from: string
  to: string
  metrics: MonitorRow['metrics']
  trend: MonitorTrend[]
}
export function observedSpan(seconds: number) {
  return seconds >= 86400
    ? (seconds / 86400).toFixed(1) + t(' 天')
    : (seconds / 3600).toFixed(1) + t(' 小时')
}
...
export interface MonitorStorage {
  ...
  unmapped_legacy: number
  database_bytes: number
  wal_bytes: number
  oldest_raw: string | null
  oldest_hourly: string | null
...
export interface MonitorCorrelation {
  ...
  comparable: number
  monitored: number
  other_provider_healthy: boolean
  nodes: string[]
  message: string
}
...
export interface MonitorActivityPage {
...
export function activityMessage(event: MonitorActivity): string {
  ...
  if (
    locale.value === 'en' &&
    ['automatic_switch', 'manual_switch'].includes(event.kind) &&
...
export const activityKind: Record<string, string> = {
  node: '节点状态',
  environment: '环境状态',
  plan: '方案变更',
  provider: 'Provider 关联',
  automatic_switch: '自动切换',
  manual_switch: '手动选择',
}
...
export function healthEvidence(metrics: MonitorRow['metrics'], window: string) {
  if (window === '1h' || metrics.readiness === 'observational')
    return { value: t('短期观察'), label: t('1 小时只展示观测指标'), scored: false }
  if (metrics.samples < 100 || metrics.score === null)
    return { value: t('积累中'), label: t('等待 100 个有效基准样本'), scored: false }
  return {
    value: metrics.score.toFixed(1),
    label: metrics.readiness === 'ready' ? t('数据充足') : t('暂定分 · 覆盖或跨度不足'),
    scored: true,
  }
}
