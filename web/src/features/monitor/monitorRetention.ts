...
export function retentionCapacity(candidateCount: number, policy: MonitorRetention, taskCount = 1) {
  const values = [
    ...
    taskCount,
    policy.raw_days,
    policy.aggregate_days,
    policy.max_raw_samples,
    policy.max_hourly,
  ]
  if (
    values.some((value) => !Number.isSafeInteger(value) || value <= 0) ||
    candidateCount > 30 * taskCount ||
    candidateCount < taskCount
  )
    return null
  if (
    policy.raw_days > 30 ||
    policy.aggregate_days < 7 ||
    policy.aggregate_days > 365 ||
    policy.aggregate_days < policy.raw_days ||
    policy.max_raw_samples < 1000 ||
    policy.max_raw_samples > retentionLimits.raw ||
    policy.max_hourly < 1000 ||
    policy.max_hourly > retentionLimits.hourly
  )
    return null
  ...
  const reserve = (required: number, cap: number) =>
    Math.min(cap, Math.ceil((required * 1.2) / 1000) * 1000)
