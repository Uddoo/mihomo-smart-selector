...
export type SwitchHistory = ReturnType<typeof useSwitchHistory>
export type HistoryPageState = Pick<
  SwitchHistory,
  'history' | 'historyLoading' | 'historyError' | 'historyLoaded' | 'historyUpdatedAt'
> & {
  configLocked: Readonly<Ref<boolean>>
  reconcile: (event: SwitchEvent) => Promise<void>
  reconcilingId: Readonly<Ref<number | null>>
  reconcileFeedback: Readonly<Ref<{ id: number; event?: SwitchEvent; error?: string } | null>>
