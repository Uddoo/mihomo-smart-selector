...
export function operationMessage(event: SwitchEvent) {
  const result =
    event.status === 'confirmed'
      ? '已回读确认切换到 ' + event.selected
      : operationLabel(event.status) + '：' + event.reason
...
export function selectionKey(storage: Storage, scan: string, node: string) {
  try {
    ...
    if (previous?.scan === scan && previous?.node === node && typeof previous.key === 'string')
      return previous.key as string
  } catch {
    /* Ignore an invalid saved attempt. */
  }
  const key = Array.from(crypto.getRandomValues(new Uint8Array(16)), (b) =>
