...
const targets = JSON.parse(
  fs.readFileSync(new URL('../../../public/connectivity-targets.json', import.meta.url)),
)
...
test('every service belongs to one product category, independent of country', () => {
  ...
  assert.deepEqual(
    targets
      .filter((target) => target.category === 'ai')
  ...
  assert.deepEqual(
    targets
      .filter((target) => target.category === 'social')
      .map((target) => target.id)
      .sort(),
    [
      'facebook',
  ...
  assert.ok(
    categories.every((category) => targets.some((target) => target.category === category.id)),
  )
})
...
test('all 48 targets define explicit evidence, redirect, cache and response contracts', () => {
  ...
  for (const t of targets) {
    ...
    assert.ok(!url.searchParams.has('_mss'))
    assert.ok(['resource', 'connectivity', 'api', 'diagnostic', 'web'].includes(t.kind))
    assert.ok(['cors', 'no-cors'].includes(t.requestMode))
    ...
    assert.ok(
      Number.isInteger(t.expected.status) && t.expected.status >= 100 && t.expected.status < 600,
    )
...
test('known redirect and endpoint regressions stay corrected', () => {
  ...
  assert.equal(byId.google.expected.status, 204)
  assert.equal(byId.youtube.expected.status, 204)
  assert.equal(byId.github.requestMode, 'cors')
  assert.notEqual(byId.takealot.kind, 'resource')
  assert.notEqual(byId.noon.kind, 'resource')
})
