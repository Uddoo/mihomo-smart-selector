...
const results = reactive<Record<string, Result>>(
  Object.fromEntries(targets.map((target) => [target.id, { phase: 'idle', samples: [] }])),
)
const selected = shallowRef<string[]>([])
const scope = shallowRef<string | null>(null),
  lastScope = shallowRef('全部服务')
const stopped = shallowRef(false),
  finishedAt = shallowRef<number | null>(null)
const onlyIssues = shallowRef(false),
  collapsed = reactive<Record<string, boolean>>({})
...
const progress = computed(() => ({
  completed: selected.value.reduce((sum, id) => sum + results[id]!.samples.length, 0),
  total: selected.value.length * ROUNDS,
}))
function makeSections(itemsInView: readonly Target[], issueIds: readonly string[]) {
  return categories
    .map((group) => {
      const items = itemsInView.filter((target) => target.category === group.id)
      const medians = items
        .map((target) => resultStats(results[target.id]!))
        .filter((stats) => stats.median !== null)
      const partial = items.filter((target) => {
        const stats = resultStats(results[target.id]!)
        return stats.success > 0 && stats.success < stats.attempted
      }).length
      // Hold the current retest in the filtered view until the entire run settles.
      const visibleTargets = onlyIssues.value
        ? items.filter(
            (target) =>
              issueIds.includes(target.id) || (busy.value && selected.value.includes(target.id)),
          )
        : items
      return {
        ...group,
        targets: items,
        visibleTargets,
        reachable: items.filter((target) => hasResponse(results[target.id]!)).length,
        partial,
        ...
        average: medians.length
          ? Math.round(medians.reduce((sum, stats) => sum + stats.median!, 0) / medians.length)
          : null,
      }
    })
...
function finish(runController: AbortController, items: readonly Target[]) {
  ...
  for (const target of items)
    if (results[target.id]!.phase !== 'complete') results[target.id]!.phase = 'stopped'
  scope.value = null
  controller = null
...
async function runItems(items: readonly Target[], runScope: string, label: string) {
  ...
  lastScope.value = label
  selected.value = items.map((target) => target.id)
  stopped.value = false
  ...
  try {
    await runProbes({
      ...
      onStart: (target) => {
        if (controller === runController) results[target.id]!.phase = 'running'
      },
...
function stop() {
  ...
  finish(
    runController,
    targets.filter((target) => selected.value.includes(target.id)),
  )
}
function setOnlyIssues(value: boolean) {
  onlyIssues.value = value
}
function toggleGroup(id: string) {
  collapsed[id] = !collapsed[id]
}

export function useConnectivity(options: {
  mineIds: () => readonly string[]
  onStart: (ids: readonly string[]) => void
}) {
  onBeforeUnmount(stop)
  const viewTargets = computed(() =>
    view.value === 'mine'
      ? targets.filter((target) => options.mineIds().includes(target.id))
      : targets,
  )
  const categoryCounts = computed(() =>
    categories.map((item) => ({
      ...item,
      count: viewTargets.value.filter((target) => target.category === item.id).length,
    })),
  )
  const activeTargets = computed(() => {
    const search = query.value.trim().toLocaleLowerCase()
    return viewTargets.value.filter(
      (target) =>
        (category.value === 'all' || target.category === category.value) &&
        (!search ||
          `${target.id} ${target.name} ${t(target.name)}`.toLocaleLowerCase().includes(search)),
    )
  })
  const scopeLabel = computed(() =>
    query.value.trim()
      ? '筛选服务'
      : (categories.find((item) => item.id === category.value)?.name ??
        (view.value === 'mine' ? '我的服务' : '全部服务')),
  )
  const issueIds = computed(() =>
    activeTargets.value
      .filter((target) => needsAttention(results[target.id]!))
      .map((target) => target.id),
  )
  const summary = computed(() => ({
    reachable: activeTargets.value.filter((target) => hasResponse(results[target.id]!)).length,
    total: activeTargets.value.length,
    issues: issueIds.value.length,
  }))
  ...
  function run(group = 'all') {
    return start(
      activeTargets.value.filter((target) => group === 'all' || target.category === group),
      group,
  ...
  function runIssues() {
    return start(
      activeTargets.value.filter((target) => issueIds.value.includes(target.id)),
      'issues',
      '异常服务',
    )
  }
  ...
  return {
    ...
    issueIds,
    view: readonly(view),
    setView: (value: ServiceView) => {
      view.value = value
    },
    category: readonly(category),
    query: readonly(query),
    categoryCounts,
    viewTotal: computed(() => viewTargets.value.length),
    scopeLabel,
    ...
    lastScope: readonly(lastScope),
    stopped: readonly(stopped),
    finishedAt: readonly(finishedAt),
    onlyIssues: readonly(onlyIssues),
