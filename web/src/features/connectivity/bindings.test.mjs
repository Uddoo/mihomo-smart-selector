...
const catalog = {
  profiles: ['chatgpt', 'github', 'apple-services', 'ecommerce-jp'].map((id) => ({ id })),
  bindings: [
    { group: 'Custom AI', profile_id: 'chatgpt', status: 'valid' },
    { group: 'Second AI', profile_id: 'chatgpt', status: 'valid' },
  ],
  suggestions: { GitHub: 'github' },
}
const scan = (id, group, profile, date, status = 'complete') => ({
  id,
  request: { target_group: group, profile_id: profile },
  profile: { id: profile },
  started_at: date,
  status,
})
...
test('missing profiles/groups, non-Selectors and stale now values never appear as a valid confirmed choice', () => {
  const result = serviceBindings(
    targets,
    [
      { ...groups[0], now: 'removed' },
      { ...groups[1], type: 'URLTest' },
    ],
    catalog,
    [],
  )
  ...
  assert.equal(
    serviceBindings(targets, groups, { ...catalog, profiles: [] }, []).chatgpt[0].status,
    'profile_missing',
  )
...
test('latest scan matches both group and service, regardless of list order or completion status', () => {
  const scans = [
    scan('wrong-service', 'Custom AI', 'github', '2026-09-15T05:00:00Z'),
...
test('changes compare with immutable per-test snapshots, not names inferred from service suggestions', () => {
  ...
  const updated = serviceBindings(
    targets,
    [{ ...groups[0], now: 'US' }, groups[1]],
    catalog,
    [],
  ).chatgpt
  ...
  assert.equal(
    withSelectionChanges(updated, snapshotSelections(updated))[0].previousSelection,
    undefined,
  )
...
test('arbitrary group names remain data and can be safely compared', () => {
  ...
  assert.equal(
    withSelectionChanges(updated, snapshotSelections(initial))[0].previousSelection,
    'JP',
  )
})
