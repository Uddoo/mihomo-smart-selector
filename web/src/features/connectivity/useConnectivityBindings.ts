...
export function useConnectivityBindings(
  source: () => {
    groups: Group[]
  ...
  const bindings = computed<BindingMap>(() =>
    available.value
      ? Object.fromEntries(
          Object.entries(accepted.value).map(([id, items]) => [
            id,
            withSelectionChanges(items, testedSelections.value[id]),
          ]),
        )
      : {},
  )
  const mineIds = computed(() =>
    targets
      .filter((target) => bindings.value[target.id]?.some((binding) => binding.valid))
      .map((target) => target.id),
  )
  const changedIds = computed(() =>
    targets
      .filter((target) =>
        bindings.value[target.id]?.some((binding) => binding.previousSelection !== undefined),
      )
      .map((target) => target.id),
  )
  function capture(ids: readonly string[]) {
    ...
    for (const id of ids)
      snapshots[id] = available.value ? snapshotSelections(bindings.value[id] || []) : {}
