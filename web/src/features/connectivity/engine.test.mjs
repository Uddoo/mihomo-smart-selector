...
test('one success and seven failures retain an explicit 1/8 success count', () => {
  const stats = resultStats({
    phase: 'complete',
    samples: [
      { outcome: 'success', ms: 80, at: 0 },
      ...Array.from({ length: 7 }, () => ({ outcome: 'timeout', ms: null, at: 0 })),
    ],
  })
...
test('timeout, network error and user cancellation are distinct', async () => {
  const hanging = async (_, { signal }) =>
    new Promise((_, reject) =>
      signal.addEventListener('abort', () => reject(signal.reason), { once: true }),
    )
  assert.equal(
    (
      await probe('https://example.com', new AbortController().signal, {
        fetcher: hanging,
        timeoutMs: 5,
      })
    ).outcome,
  ...
  assert.equal(
    await probe('https://example.com', controller.signal, {
      fetcher: () => assert.fail('must not fetch'),
    }),
    null,
  )
})
...
test('stopping a pool prevents queued work and ignores in-flight late results', async () => {
  ...
  let samples = 0
  await runProbes({
    targets: Array.from({ length: 12 }, (_, i) => ({ id: String(i), url: '' })),
