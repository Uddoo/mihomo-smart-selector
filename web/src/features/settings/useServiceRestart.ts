  ...
  async function refresh() {
    ...
    try {
      ...
      if (!lifetime.signal.aborted)
        error.value = cause instanceof Error ? cause.message : '无法读取服务状态，请重试'
    }
  }
    ...
    try {
      ...
      if (before.status !== 'restarting') {
        try {
          await api('/service/restart', {
            method: 'POST',
            body: JSON.stringify({ instance_id: before.instance_id, confirm: true }),
      ...
      while (!lifetime.signal.aborted && Date.now() < deadline) {
        ...
        try {
          ...
          /* The listener is briefly unavailable while workers drain. */
        }
      }
      if (!lifetime.signal.aborted)
        throw new Error('尚未确认服务恢复，请刷新状态；若仍不可用，请查看启动窗口的错误信息。')
    } catch (cause) {
      if (!lifetime.signal.aborted)
        error.value = cause instanceof Error ? cause.message : '重启失败，请检查服务状态'
    } finally {
      busy.value = false
    }
  }
