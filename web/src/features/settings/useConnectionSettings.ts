...
export function useConnectionSettings() {
  ...
  const dirty = computed(
    () =>
      !!state.value &&
      (draft.value.controller !== state.value.saved.controller ||
        draft.value.request_timeout_seconds !== state.value.saved.request_timeout_seconds ||
  ...
  function apply(value: ConnectionState) {
    state.value = value
    draft.value = {
      controller: value.saved.controller,
      request_timeout_seconds: value.saved.request_timeout_seconds,
    }
    secretAction.value = 'keep'
    secret.value = ''
  }
  ...
  async function run(action: 'load' | 'save' | 'test', useServer = false) {
    ...
    const payload = useServer
      ? { revision: state.value?.revision, use_server: true }
      : {
          revision: state.value?.revision,
          ...draft.value,
          secret_action: secretAction.value,
          secret: secretAction.value === 'replace' ? secret.value : '',
        }
    try {
      if (action === 'load')
        apply(await api<ConnectionState>('/connection', { signal: request.signal }))
      else if (action === 'test') {
        ...
        apply(
          await api<ConnectionState>('/connection', {
            method: 'PUT',
            body: JSON.stringify(payload),
            signal: request.signal,
          }),
        )
        notice.value = state.value?.restart_required
          ? '连接配置已保存，请重启本服务后生效。'
          : '连接配置已保存，与当前连接一致。'
      }
    } catch (e) {
  ...
  return {
    ...
    dirty,
    sourceLabel,
    reload: () => (confirmReload() ? run('load') : Promise.resolve()),
    save: () => run('save'),
    test: () => run('test'),
    restore: () => (confirmReload() ? run('save', true) : Promise.resolve()),
  }
}
