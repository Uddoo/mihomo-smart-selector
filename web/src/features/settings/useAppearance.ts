...
export function useAppearance() {
  ...
  function storageChanged(event: StorageEvent) {
    ...
    syncingStorage = true
    try {
      if (event.key === null || event.key === 'mss-palette')
        palette.value = resolvePalette(event.newValue)
      if (event.key === null || event.key === 'mss-theme')
        mode.value = resolveAppearanceMode(event.newValue)
    } finally {
  ...
  function toggleTheme() {
    mode.value = theme.value === 'light' ? 'dark' : 'light'
  }
