...
import type { NodeResult } from '../../shared/types/models'
...
export function rankResults(results: readonly NodeResult[]): NodeResult[] {
  return [...results]
    .sort(
      (a, b) =>
        Number(b.stage === 'refined') - Number(a.stage === 'refined') ||
        b.score - a.score ||
...
export function evidence(result: NodeResult) {
  ...
  return t('{p0} / {p1} 次成功 · {p2}（初筛 {p3} + 复测 {p4}）{p5}', {
    p0: successes,
    p1: samples.length,
    p2: result.stage === 'refined' ? t('已复测') : t('仅初筛'),
    p3: result.screening_samples ?? samples.length,
    p4: result.refinement_samples ?? 0,
    p5: successes < 20 ? t(' · 样本较少，P95 仅供参考') : '',
  })
}
...
export function candidateComparison(
  candidate: NodeResult | undefined,
  current: NodeResult | undefined,
  now: number,
): string {
  ...
  if (!(candidate.p95_ms && candidate.p95_ms > 0 && current.p95_ms && current.p95_ms > 0))
    return t('暂无可比较的 P95 时延')
  const delta = Math.round(candidate.p95_ms - current.p95_ms)
  const latency =
    delta === 0 ? t('P95 相同') : `P95 ${delta < 0 ? t('降低') : t('增加')} ${Math.abs(delta)} ms`
  const success = (candidate.success_rate - current.success_rate) * 100
  return t('{p0} · 成功率{p1}', {
    p0: latency,
    p1:
      Math.abs(success) < 0.05
        ? t('持平')
        : t('{p0} {p1} 个百分点', {
            p0: success > 0 ? t('增加') : t('降低'),
            p1: Math.abs(success).toFixed(1),
          }),
  })
}
