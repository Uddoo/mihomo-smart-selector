...
export function createScanTransport(deps: ScanTransportDependencies) {
  ...
  function valid(read: number) {
    return active && generation === read
  }
  function clear() {
    ++generation
    active = false
  ...
  function scheduleTick(read: number) {
    ...
    if (valid(read))
      tick = setTimeout(
        ...
        live ? 15000 : Math.min(30000, 3000 * 2 ** Math.min(failures, 4)),
      )
  }
  function refresh(): Promise<void> {
    ...
    pending = (async () => {
      try {
        ...
        failures = 0
        deps.onSnapshot(value)
        if (value.status !== 'running') {
          clear()
          deps.onMode('idle')
        }
      } catch (error) {
        if (read === generation && (!abort.signal.aborted || timedOut)) {
          failures++
          deps.onError(
            timedOut
              ? '读取扫描超时，正在重试'
  ...
  function connect(read: number) {
    ...
    function event(value: ScanEvent) {
      ...
      if (['completed', 'error', 'warning'].includes(value.kind)) {
        void refresh()
        return
      }
      ...
      if (!batch)
        batch = setTimeout(() => {
          batch = undefined
          if (valid(read) && !abort.signal.aborted) {
            const values = events
            events = []
            deps.onEvents(values)
          }
        }, 250)
    }
    activity()
    void deps
      .stream(id, abort.signal, event, activity)
      .catch(() => {})
      .finally(() => {
        if (!valid(read)) return
        clearTimeout(heartbeat)
        ...
        retry = setTimeout(
          () => connect(read),
          Math.min(30000, 1000 * 2 ** Math.min(reconnects++, 5)),
        )
      })
  }
  function resume() {
