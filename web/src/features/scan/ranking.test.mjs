...
test('refined results precede single-sample screening results', () => {
  ...
  assert.deepEqual(
    rankResults([screened, refined]).map((r) => r.name),
    ['refined', 'screened'],
  )
})
...
const node = (name, score, success_rate = 1, p95_ms = 100) => ({
  name,
  score,
  success_rate,
  p95_ms,
...
test('later high scoring results move above existing nodes before scan completion', () => {
  ...
  assert.deepEqual(
    rankResults([first]).map((x) => [x.name, x.rank]),
    [['slow', 1]],
  )
  const snapshot = Object.freeze([Object.freeze(first), Object.freeze(second)])
  assert.deepEqual(
    rankResults(snapshot).map((x) => [x.name, x.rank]),
    [
      ['fast', 1],
      ['slow', 2],
    ],
...
test('matches backend tie breaking and preserves order of exact ties', () => {
  const input = [
    node('low-success', 70, 0.9, 50),
    node('high-p95', 70, 1, 120),
    node('tie-a', 70),
    node('tie-b', 70),
  ]
  assert.deepEqual(
    rankResults(input).map((x) => x.name),
    ['tie-a', 'tie-b', 'high-p95', 'low-success'],
  )
})
...
test('updated verification scores replace stale ranks and empty scans stay empty', () => {
  ...
  const input = [
    { ...node('old-first', 40), rank: 1 },
    { ...node('new-first', 80), rank: 2 },
  ]
  assert.deepEqual(
    rankResults(input).map((x) => [x.name, x.rank]),
    [
      ['new-first', 1],
      ['old-first', 2],
    ],
...
test('comparison never treats absent or expired latency as an improvement', () => {
  ...
  assert.match(
    candidateComparison(candidate, { ...current, expires_at: new Date(now).toISOString() }, now),
    /已过期/,
  )
  assert.match(
    candidateComparison({ ...candidate, expires_at: new Date(now).toISOString() }, current, now),
    /已过期/,
  )
  assert.match(candidateComparison(current, current, now), /当前正在使用/)
  assert.match(
    candidateComparison({ ...candidate, success_rate: 0.8 }, current, now),
    /成功率降低 20.0/,
  )
})
