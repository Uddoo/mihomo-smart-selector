...
import type { NodeResult, Scan } from '../../shared/types/models'
...
const props = defineProps<{
  ...
  filtered: boolean
  demo: boolean
  selectionReason: (row: NodeResult) => string
}>()
...
const content = computed(() =>
  snapshot.value
    ? buildResultExport(snapshot.value, {
        format: format.value,
        fields: fields.value,
        filter: filter.value,
...
watch(
  [format, filter, fields, includeNames],
  () => {
    message.value = ''
    failed.value = false
  },
...
async function copy() {
  ...
  try {
    ...
    failed.value = true
    message.value = '无法访问剪贴板，已选中预览内容，请手动复制。'
    preview.value?.focus()
  ...
  >
    <template v-if="snapshot">
      ...
      <p class="export-description">{{
        t('固定打开时的结果快照，覆盖当前搜索的全部分页；预览、复制与下载使用相同内容。')
      }}</p>
      <p class="snapshot-time"
        >{{ t('快照时间') }} {{ formatDate(new Date(snapshot.at)) }}
        <span v-if="snapshot.demo"> · {{ t('示例数据') }}</span></p
      >
      <p
        v-if="snapshot.scan.status !== 'complete'"
      ...
      <div class="export-options">
        ...
        <label class="export-option"
          >{{ t('导出筛选')
            ...
            v-model="filter"
            class="export-select"
            ><option value="all">{{ t('全部搜索结果') }}</option
            ><option value="selectable">{{ t('快照时可选择的结果') }}</option
            ><option value="refined">{{ t('已复测结果') }}</option></select
          ></label
        >
      </div>
      <label class="name-choice"
        ><input
          v-model="includeNames"
          type="checkbox"
        />{{ t('包含真实节点、Provider 和策略组名称') }}</label
      >
      <p class="export-hint">{{
        t('默认使用一致别名；不导出探测地址或原始错误。可选择状态仅代表快照时的检查结果。')
      }}</p>
      <fieldset class="export-fields"
        ><legend>{{ t('结果字段') }}</legend>
        <div class="field-actions"
          ><button @click="fields = exportFields.map((field) => field.id)">{{
            t('全选字段')
          }}</button
          ><button @click="fields = []">{{ t('清空字段') }}</button></div
        >
      ...
      <p class="export-hint">{{
        t('服务、模式、测量边界和快照时间始终保留。CSV 使用固定列名，文件时间采用 ISO 8601。')
      }}</p>
      <div class="preview-heading"
        ><label for="scan-export-preview">{{ t('内容预览') }}</label
        ><span aria-live="polite">{{ t('{count} 条结果', { count }) }}</span></div
      >
      <textarea
        id="scan-export-preview"
        ref="preview"
      ...
      />
      <p
        v-if="!fields.length || !count"
        class="export-warning"
        role="status"
        >{{ t(!fields.length ? '请至少选择一个结果字段。' : '此筛选下没有可导出的结果。') }}</p
      >
      <p
        v-if="message"
        :class="failed ? 'export-warning' : 'export-success'"
        role="status"
        >{{ t(message) }}</p
      >
