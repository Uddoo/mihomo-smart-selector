...
import { fetchAPI, setAPIToken } from '../../shared/api/api.ts'
...
function setup(t, overrides = {}) {
  ...
  const dependencies = {
    ...
    stream: (id, signal, event, activity) =>
...
test('SSE coalesces updates and only a REST snapshot confirms completion', async (t) => {
  ...
  s.transport.start('a')
  await flush()
  s.streams[0].event({ kind: 'connected' })
  await flush()
...
test('verification warnings fetch a fresh snapshot without completing the scan', async (t) => {
  ...
  await flush()
  warnings = [
    { code: 'probe_restore_failed', phase: 'egress', group: 'probe', message: 'restore failed' },
  ]
  s.streams[0].event({ kind: 'warning' })
  await flush()
...
test('late snapshots and events from an older scan cannot overwrite the active scan', async (t) => {
  ...
  const s = setup(t, {
    snapshot: (id) =>
      id === 'old' ? new Promise((r) => (resolveOld = r)) : Promise.resolve(snapshot(id)),
  })
  s.transport.start('old')
  await flush()
  s.transport.start('new')
  await flush()
  ...
  const s = setup(t, {
    snapshot: (id) =>
      ++reads === 1
        ...
        : Promise.resolve({ ...snapshot(id), revision: 2 }),
  })
  s.transport.start('a')
  await flush()
  s.streams[0].event({ kind: 'candidate-complete', result: { name: 'stale' } })
  resolveRead({ ...snapshot('a'), revision: 1 })
  await flush()
  t.mock.timers.tick(250)
  await flush()
  assert.equal(s.batches.length, 0)
...
test('stream loss falls back to polling and reconnects without simultaneous snapshots', async (t) => {
  ...
  s.streams[0].event({ kind: 'connected' })
  await flush()
  s.streams[0].reject(new Error('connection lost'))
  await flush()
...
test('hidden/offline suspension cancels work and resuming recovers completed scans', async (t) => {
  ...
  const s = setup(t, {
    snapshot: async (id) => {
      reads++
      return snapshot(id, done ? 'complete' : 'running')
    },
  })
  s.transport.start('a')
  await flush()
...
test('first streamed results tolerate omitted initial results and replace repeated node updates', () => {
  ...
  const first = applyScanEvents(initial, [
    { kind: 'candidate-complete', result: { name: 'n', score: 10 } },
  ])
  assert.equal(first.results.length, 1)
  const next = applyScanEvents(first, [
    { kind: 'strict-verified', result: { name: 'n', score: 20 } },
    { kind: 'candidate-complete', result: { name: 'b', score: 15 }, progress: { completed: 2 } },
  ])
  assert.deepEqual(
    next.results.map((row) => row.score),
    [20, 15],
  )
...
test('concurrent refresh callers share one in-flight snapshot', async (t) => {
  ...
  const s = setup(t, {
    snapshot: (id) => {
      reads++
      return new Promise((resolve) => (finish = () => resolve(snapshot(id))))
    },
  })
  s.transport.start('a')
  await flush()
  const first = s.transport.refresh(),
    second = s.transport.refresh()
