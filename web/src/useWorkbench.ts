...
export function useWorkbench() {
  ...
  watch(page, () => { ++serviceNavigationRevision; openingServiceScan.value = false }, {flush: 'sync'})
  ...
  function openServiceCandidates(target: string) {
    if (configLocked.value || loading.value || !discoveryValid.value || !groups.value.some(item => item.name === target)) return
    catalog.clear(); catalog.groupScope.value = target; catalog.filters.scope = 'all'; catalog.page.value = 1
    page.value = 'nodes'
  }

  async function prepareServiceVerification(target: string, profileID: string) {
    if (page.value !== 'connectivity' || configLocked.value || loading.value || openingServiceScan.value
      || !discoveryValid.value || !groups.value.some(item => item.name === target && item.type === 'Selector')
      || !services.value?.bindings.some(item => item.group === target && item.profile_id === profileID && item.status === 'valid')) return
    ...
    serviceID.value = profileID; areas.value = []; providerSet.value = []; mode.value = 'stable'
    openingServiceScan.value = false; page.value = 'scan'
  }

  async function openServiceScan(id: string) {
    if (page.value !== 'connectivity' || configLocked.value || loading.value || openingServiceScan.value || !recent.value.some(item => item.id === id)) return
    const revision = ++serviceNavigationRevision
    const currentIntent = () => revision === serviceNavigationRevision && page.value === 'connectivity'
    openingServiceScan.value = true
    try {
      if (!await session.open(id, currentIntent)) return
      await restoreForm(currentIntent)
      if (!currentIntent()) return
      focusedName.value = ''; pendingChoice.value = null; page.value = 'scan'
    } catch (error) {
      if (currentIntent()) failure.value = error instanceof Error ? error.message : '无法打开扫描'
    } finally { if (currentIntent()) openingServiceScan.value = false }
  }
