import {t, compareText} from './i18n.ts'
...
export function regionSourceLabel(source: string): string {
  return ({manual: t('配置中手动指定'), 'name-inferred': t('根据节点名称推断'), unknown: t('未识别到地区'), ambiguous: t('名称线索需要确认'), dynamic: t('自动线路，地区可能变化'), 'not-applicable': t('此类条目不适用地区推断')} as Record<string, string>)[source] || t('其他推断来源')
}
...
export function entryKindLabel(kind?: NodeSummary['entry_kind']): string {
  return kind ? ({proxy: t('代理节点'), builtin: t('内置出站'), 'subscription-info': t('疑似订阅提示'), dynamic: t('动态线路')})[kind] : t('代理节点')
}
...
export function catalogRegionLabel(node: NodeSummary, regionLabel: (code?: string) => string): string {
  return node.inferred_region ? regionLabel(node.inferred_region) : t(statusLabels[regionStatus(node)])
}
...
export function regionReasonLabel(node: NodeSummary): string {
  if (node.region_reason === 'transit') return t('名称含中转或转接线索，命中的地区可能属于入口或中转地，暂不指定出口地区。')
  if (node.region_reason === 'conflicting-cues') return t('名称或旗帜指向多个地区，暂不自动选择；可通过配置中的 region_overrides 手动指定。')
  if (node.entry_kind === 'subscription-info') return t('仅根据名称判断为疑似订阅提示，条目仍保留，可在全部条目中查看。')
  if (node.entry_kind === 'builtin') return t('直连、拒绝等内置出站不代表固定地区的代理服务器。')
  if (node.entry_kind === 'dynamic') return t('自动线路的出口可能随服务方调度变化，名称无法提供固定地区。')
  if (!node.inferred_region) return t('尚未命中地区词典，可补充地区别名或使用 region_overrides 手动指定。')
