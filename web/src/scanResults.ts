import {t, translateMessage} from './i18n.ts'
import {hasJitterEvidence} from './ranking.ts'
import type {NodeResult, Scan} from './models.ts'
...
const latency = (value?: number) => value && value > 0 && Number.isFinite(value) ? value : Infinity
function ascending(a: number, b: number) { return a === b ? 0 : a < b ? -1 : 1 }
...
function strictOrder(status: string) {
  ...
  return ['not_requested', 'not_configured', 'not_checked', 'not_run_limit', 'unknown', 'unverified', ''].includes(status || '') ? 1 : 2
}

// View ordering never rewrites the original score rank or changes selection gates.
export function orderResults(results: readonly NodeResult[], view: ResultView, now: number): NodeResult[] {
  ...
  return results.map((row, index) => ({row, index})).sort((a, b) => {
    ...
    let order = 0
    if (view === 'stability') {
      const jitter = (r: NodeResult) => hasJitterEvidence(r) && finite(r.jitter_ms, -1) >= 0 ? r.jitter_ms! : Infinity
      ...
      order = strictOrder(x.strict_verification_status) - strictOrder(y.strict_verification_status)
...
export function sampleCounts(result: NodeResult) {
  ...
  return {total: samples.length, successful: samples.filter(s => !s.error && latency(s.delay_ms) < Infinity).length}
}
export const measurementLimit = '时延与可达性不代表登录、解锁、播放或长连接质量；小样本 P95 仅供参考。'
export const comparisonLimit = '跨扫描比较前，请确认服务、探测目标、模式、采样和网络环境一致。'
export function scanStatusLabel(status: Scan['status']) {
  return t(({running: '进行中', complete: '已完成', cancelled: '已停止', failed: '失败', interrupted: '已中断'})[status])
}
...
export const exportFields = [
  {id: 'rank', label: '综合排名'}, {id: 'name', label: '节点名称'}, {id: 'provider', label: 'Provider'},
  {id: 'region', label: '推断地区'}, {id: 'verified_region', label: '已验证地区'},
  {id: 'score', label: '性能评分'}, {id: 'success_rate', label: '成功率 (%)'},
  {id: 'p50_ms', label: 'P50 (ms)'}, {id: 'p95_ms', label: 'P95 (ms)'}, {id: 'jitter_ms', label: '抖动 (ms)'},
  {id: 'samples', label: '采样次数'}, {id: 'successful_samples', label: '成功次数'},
  {id: 'screening_samples', label: '初筛次数'}, {id: 'refinement_samples', label: '复测次数'},
  {id: 'stage', label: '采样阶段'}, {id: 'reachability', label: '可达性'},
  {id: 'strict', label: '严格验证'}, {id: 'restriction', label: '服务限制'},
  {id: 'region_verification', label: '地区验证'}, {id: 'measured_at', label: '测量时间'},
  {id: 'expires_at', label: '有效期至'}, {id: 'validity', label: '时效状态'},
...
export interface ExportOptions {format: ExportFormat; fields: readonly ExportField[]; filter: ExportFilter; includeNames: boolean}

export function exportRows(snapshot: ExportSnapshot, filter: ExportFilter) {
  const allowed = new Set(snapshot.selectable)
  return snapshot.rows.filter(row => filter === 'all' || (filter === 'refined' ? row.stage === 'refined' : allowed.has(row.name)))
}
function iso(value?: string) { return value && Number.isFinite(Date.parse(value)) ? new Date(value).toISOString() : '' }
function numberCell(value?: number) { return Number.isFinite(value) ? String(value) : '' }
...
export function buildResultExport(snapshot: ExportSnapshot, options: ExportOptions) {
  ...
  const metadata = [
    ['scan_id', '扫描 ID', scan.id], ['scan_status', '扫描状态', scan.status],
    ['service_id', '服务 ID', scan.profile.id], ['service', '测试服务', translateMessage(scan.profile.label)],
    ['group', '目标策略组', options.includeNames ? scan.request.target_group : 'Group 1'],
    ['mode', '模式', scan.request.mode], ['transport_scope', '传输范围', translateMessage(scan.profile.transport_scope)],
    ['started_at', '开始时间', iso(scan.started_at)], ['completed_at', '结束时间', iso(scan.completed_at)],
    ['snapshot_at', '快照时间', new Date(at).toISOString()], ['view', '排序视图', snapshot.view],
    ['result_scope', '结果范围', snapshot.filtered ? 'search_matches' : 'all_results'],
    ['result_filter', '导出筛选', options.filter], ['data_source', '数据来源', snapshot.demo ? 'mock' : 'controller'],
    ['warning_count', '验证警告数', String(scan.warnings?.length || 0)],
    ['measurement_note', '测量边界', t(measurementLimit)], ['comparison_note', '比较条件', t(comparisonLimit)],
  ]
  const cells = rows.map(row => {
    const counts = sampleCounts(row)
    const values: Record<ExportField, string> = {
      rank: String(row.rank), name: options.includeNames ? row.name : nodeAliases.get(row.name) || 'Node',
      provider: options.includeNames ? row.provider || '' : providerAliases.get(row.provider) || '',
      region: row.inferred_region || '', verified_region: row.verified_region || '', score: numberCell(row.score),
      ...
      jitter_ms: hasJitterEvidence(row) && finite(row.jitter_ms, -1) >= 0 ? numberCell(row.jitter_ms) : '',
      samples: String(counts.total), successful_samples: String(counts.successful), stage: row.stage || 'unknown',
      screening_samples: numberCell(row.screening_samples), refinement_samples: numberCell(row.refinement_samples),
      reachability: row.reachability_status || 'unknown', strict: row.strict_verification_status || 'unknown',
      restriction: row.restriction_status || 'unknown', region_verification: row.region_verification_status || 'unknown',
      measured_at: iso(row.measured_at), expires_at: iso(row.expires_at), validity: validity(row, at),
      selectable: String(snapshot.selectable.includes(row.name)),
    }
    return fields.map(field => values[field.id])
  })
  if (options.format === 'csv') {
    return [[...fields.map(field => field.id), ...metadata.map(item => item[0])],
      ...cells.map(row => row.concat(metadata.map(item => item[2])))].map(row => row.map(csvCell).join(',')).join('\r\n') + '\r\n'
  }
  const line = (values: string[]) => '| ' + values.map(markdownCell).join(' | ') + ' |'
  return `# ${t('扫描结果')}\n\n` + metadata.map(([, label, value]) => `- ${t(label)}: ${markdownCell(value || '—')}`).join('\n')
    + '\n\n' + [line(fields.map(field => t(field.label))), '| ' + fields.map(() => '---').join(' | ') + ' |', ...cells.map(line)].join('\n') + '\n'
}
