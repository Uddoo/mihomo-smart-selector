...
<template>
  <div class="history-statuses">
    <span :class="['history-status', 'is-' + presentation.tone]"><component :is="presentation.icon" :size="14" aria-hidden="true"/>{{ t(operationLabel(status)) }}</span>
    <span v-if="!persisted" class="history-audit"><TriangleAlert :size="14" aria-hidden="true"/>{{ t('审计保存异常') }}</span>
  </div>
</template>
