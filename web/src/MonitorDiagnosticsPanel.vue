...
const props = defineProps<{window: string; mode: 'events' | 'settings'; active: boolean; candidateCount: number; taskCount: number; taskId: string}>()
const emit = defineEmits<{locate: [event: MonitorActivity]; retentionRead: [policy: MonitorRetention]}>()
...
const capacityWarning = computed(() => !!policy.value && !!retentionCapacity(props.candidateCount, policy.value, props.taskCount)?.insufficient)
const events = ref<MonitorActivity[]>([]), cursor = ref(''), error = ref(''), message = ref(''), busy = ref(false), loadingMore = ref(false)
const includeNames = ref(false), exportFrom = ref(''), exportTo = ref('')
...
async function loadEvents(more = false) {
  ...
  try {
    const response = await api<MonitorActivityPage>(`${taskPath(props.taskId)}/incidents?window=${props.window}&to=${encodeURIComponent(eventTo)}&limit=50${more ? '&cursor=' + encodeURIComponent(cursor.value) : ''}`, {signal: eventController.signal})
...
async function poll(generation: number) {
  ...
  try {
    ...
    if (props.mode === 'settings') {
      ...
      error.value = ''; storage.value = usage; emit('retentionRead', usage.policy); if (!policy.value) policy.value = {...usage.policy}
    } else {
      const related = await api<MonitorCorrelation[]>(taskPath(props.taskId) + '/correlations', {signal: pollController.signal})
      if (disposed || generation !== pollGeneration) return
...
async function savePolicy() {
  ...
  try { policy.value = await api<MonitorRetention>('/monitor/retention', {method: 'PUT', body: JSON.stringify(policy.value)}); emit('retentionRead', policy.value); message.value = '保留策略已保存，按小时执行；未聚合的唯一证据不会被删除。'; storage.value = await api<MonitorStorage>('/monitor/storage') }
  catch (e) { error.value = e instanceof Error ? e.message : '保存失败' }
  finally { busy.value = false }
}
async function reloadPolicy() { try { storage.value = await api<MonitorStorage>('/monitor/storage'); policy.value = {...storage.value.policy}; emit('retentionRead', storage.value.policy); error.value = '' } catch (e) { error.value = e instanceof Error ? e.message : '读取失败' } }
...
async function exportBundle() {
  ...
  try {
    ...
    const blob = await downloadAPI(taskPath(props.taskId) + '/diagnostics', {from: new Date(exportFrom.value).toISOString(), to: new Date(exportTo.value).toISOString(), include_names: includeNames.value, include_legacy: false})
...
onMounted(() => { exportTo.value = localDate(new Date()); exportFrom.value = localDate(new Date(Date.now() - 3600000)); initialFrom = exportFrom.value; initialTo = exportTo.value; document.addEventListener('visibilitychange', restartPoll); globalThis.addEventListener('online', restartPoll); globalThis.addEventListener('offline', restartPoll); restartPoll() })
onBeforeUnmount(() => { disposed = true; clearTimeout(timer); eventController?.abort(); pollController?.abort(); document.removeEventListener('visibilitychange', restartPoll); globalThis.removeEventListener('online', restartPoll); globalThis.removeEventListener('offline', restartPoll) })
</script>

<template>
  <section class="diagnostic-panel" :aria-label="mode === 'events' ? t('事件时间线') : t('诊断与存储')">
    <h3>{{ mode === 'events' ? t('事件时间线') : t('诊断与存储') }}</h3><p v-if="mode === 'settings'">{{ t('保留策略由所有监控任务共享；诊断包仅包含当前任务。') }}</p>
