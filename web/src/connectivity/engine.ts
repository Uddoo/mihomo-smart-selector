...
export function resultStats(result: ResultView) {
  ...
  return {success: success.length, attempted: result.samples.length, median: median(success.map(sample => sample.ms!))}
}
...
export async function probe(input: Target | string, signal: AbortSignal, options: {fetcher?: typeof fetch; timeoutMs?: number; now?: () => number} = {}): Promise<Sample | null> {
  if (signal.aborted) return null
  const controller = new AbortController(), now = options.now ?? (() => performance.now())
  ...
  const onPolicyViolation = (event: SecurityPolicyViolationEvent) => {
    ...
    if (event.disposition === 'enforce' && event.effectiveDirective === 'connect-src') {
      try { if (new URL(event.blockedURI).origin === target?.origin) cspBlocked = true } catch { /* no URL supplied */ }
    }
  }
  ...
  try {
    ...
    const response = await (options.fetcher ?? fetch)(target.href, {
      method: 'GET', mode: policy?.requestMode ?? 'no-cors', credentials: 'omit', cache: 'no-store',
      referrerPolicy: 'no-referrer', redirect: policy?.redirect ?? 'follow', signal: controller.signal,
    })
    if (signal.aborted) return null
    ...
    if (!policy || policy.requestMode === 'no-cors' || response.type === 'opaque') {
      return {outcome: 'success', ms, at: Date.now(), level: policy?.kind === 'resource' ? 'resource' : 'reachable'}
    }
    if (response.type === 'opaqueredirect' || !response.status) return {outcome: 'unverifiable', ms: null, at: Date.now(), reason: 'browser'}
    status = response.status
    const mismatch = (reason: Sample['reason']): Sample => ({outcome: 'mismatch', ms: null, at: Date.now(), status: response.status, reason})
    if (response.status !== policy.expected.status) return mismatch('status')
    if (policy.expected.contentType && !(response.headers.get('Content-Type') ?? '').toLowerCase().startsWith(policy.expected.contentType)) return mismatch('content-type')
    if (policy.expected.body !== 'none' && !bodyMatches(policy.expected, await readBoundedBody(response))) return mismatch('body')
    if (signal.aborted) return null
    return {outcome: 'success', ms, at: Date.now(), level: 'verified', status: response.status}
  } catch (error) {
    if (signal.aborted) return null
    if (error instanceof Error && error.message === 'response-too-large') return {outcome: 'mismatch', ms: null, at: Date.now(), status, reason: 'response-too-large'}
    if (!timedOut && (cspBlocked || policy?.requestMode === 'cors')) return {outcome: 'unverifiable', ms: null, at: Date.now(), status, reason: cspBlocked ? 'csp' : 'browser'}
...
type RunnerOptions = {
  targets: readonly Target[]; signal: AbortSignal;
  onStart?: (target: Target) => void;
  onSample: (target: Target, sample: Sample) => void;
  measure?: typeof probe; concurrency?: number; rounds?: number;
}

export async function runProbes({targets, signal, onStart, onSample, measure = probe, concurrency = CONCURRENCY, rounds = ROUNDS}: RunnerOptions) {
