...
export function useSampleDetails(details: Readonly<Ref<HTMLDetailsElement | null>>, panel: Readonly<Ref<HTMLDivElement | null>>) {
  ...
  function updatePosition() {
    ...
    if (card.bottom < top || card.top > top + height) { closeDetails(false); return }
    ...
    position.value = {
      ...
      width: `${panelWidth}px`, maxHeight: `${height - 24}px`,
    }
  }
  ...
  function outside(event: Event) {
    if (event.target instanceof Node && !details.value?.closest('article')?.contains(event.target)) closeDetails(false)
  }
  function cleanup() {
    cancelAnimationFrame(frame)
