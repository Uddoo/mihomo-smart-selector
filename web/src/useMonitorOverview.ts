...
export function useMonitorOverview(window: Ref<string>, taskID: string, active: Ref<boolean> = shallowRef(true)) {
  const data = shallowRef<MonitorOverview | null>(null)
  const failure = shallowRef(''), lastUpdated = shallowRef('')
  ...
  async function refresh() {
    ...
    try {
      const result = await api<MonitorOverview>(taskPath(taskID) + '/overview?window=' + window.value, {signal: controller.signal})
  ...
  function resume() { failures = 0; cancel(); if (active.value || !data.value) void refresh() }
  watch(window, resume)
  watch(active, resume)
  onMounted(() => {
