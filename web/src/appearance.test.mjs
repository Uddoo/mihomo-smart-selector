...
import {palettes, readAppearance, resolvePalette, resolveAppearanceMode, resolveTheme} from './appearance.ts'
  ...
  for (const palette of [...palettes.map(p => p.id), 'invalid', null]) {
    for (const mode of ['light', 'dark', 'system', 'invalid', null]) {
      for (const dark of [false, true]) {
        ...
        assert.equal(document.documentElement.dataset.theme, resolveTheme(resolveAppearanceMode(mode), dark))
      }
    }
  }
})
...
test('denied storage does not prevent appearance initialization', () => {
  ...
  try {
    ...
    Object.defineProperty(context, 'localStorage', {get() { throw new Error('Storage denied') }})
    runInNewContext(readFileSync(new URL('../public/appearance-init.js', import.meta.url), 'utf8'), context)
