...
function initialLocale(): Locale {
  ...
  try {
    saved = globalThis.localStorage?.getItem(storageKey) ?? null
  } catch {
...
if (typeof window !== 'undefined')
  window.addEventListener('storage', (event) => {
    if (event.key === storageKey)
      activeLocale.value = resolveLocale(event.newValue, navigator.languages)
  })
...
export function formatNumber(value: number, options: Intl.NumberFormatOptions = {}): string {
  ...
  if (!formatter) {
    formatter = new Intl.NumberFormat(intlLocale.value, options)
...
export function formatDate(
  ...
  style: 'datetime' | 'time' | 'date' = 'datetime',
): string {
  ...
  if (!formatter) {
    const options: Intl.DateTimeFormatOptions = {
      ...
      ...(style !== 'date'
        ? ({ hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false } as const)
        : {}),
    }
...
export function formatRegion(code?: string, fallback?: string): string {
  ...
  if (!names) {
    names = new Intl.DisplayNames([intlLocale.value], { type: 'region' })
    regionNames.set(intlLocale.value, names)
  }
...
export function t(source: string | null | undefined, params: MessageParams = {}): string {
  ...
  return interpolate(
    target === undefined
      ? source
      : source.slice(0, source.indexOf(key)) +
          target +
...
export function translateMessage(source: string | null | undefined, depth = 0): string {
  ...
  for (const suffix of messageSuffixes) {
    if (source.endsWith(suffix))
      return translateMessage(source.slice(0, -suffix.length), depth + 1) + t(suffix)
  }
    ...
    if (match) {
      const params = Object.fromEntries(
        message.names.map((name, index) => [
          name,
          message.translated.includes(name)
            ? translateMessage(match[index + 1], depth + 1)
            : match[index + 1]!,
        ]),
      )
