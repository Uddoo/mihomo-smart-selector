...
import { activityMessage, healthEvidence } from '../features/monitor/monitoring.ts'
import { providerLabel } from '../features/node-catalog/nodeCatalog.ts'
...
test('computed copy, stored notices and evidence update without replacing state', () => {
  ...
  const rendered = computed(() => [
    ...
    healthEvidence({ samples: 12, score: null }, '24h').value,
  ])
  assert.deepEqual(rendered.value, ['扫描工作台', notice, '积累中'])
  setLocale('en')
  assert.deepEqual(rendered.value, [
    'Scan workbench',
    'Controller confirmed switch to 健康 {p0}',
    'Collecting data',
  ])
  setLocale('zh-CN')
...
test('known backend notices translate only message fields and preserve unknown diagnostics', () => {
  ...
  assert.equal(
    translateMessage('读取超时，请重试；请检查选择历史，重试会沿用同一次请求。'),
    'The request timed out. Please retry.; check switch history. A retry will reuse the same request.',
  )
  assert.equal(
    activityMessage({
      kind: 'manual_switch',
      previous: '健康；A',
      selected: '失败',
      message: '健康；A → 失败；已回读确认切换到 失败',
    }),
    '健康；A → 失败; Controller confirmed switch to 失败',
  )
})
