...
<template>
  <section class="monitor-page" :aria-label="t('持续监控')">
    ...
    <MonitorPlanEditor v-if="editing" v-show="tab === 'settings'" :plan="plan" :draft="draft" :tasks="tasks" :scheduler="scheduler" @draft="draft = $event" :retention-policy="retentionPolicy" :groups="groups" :services="services" :disabled="busy" @busy="busy = $event" @saved="saved" @cancel="cancelEdit"/>
    <template v-if="plan && data">
      <p v-if="data.plan?.revision !== plan.revision" role="status">{{ t('方案已更新，正在读取最新观测。') }}</p>
      ...
      <MonitorHistoryPanel :task-id="plan.task_id" :overview="data" :window="windowRange" :series-id="selectedSeries" :focus-event="focusedEvent" @select-series="selectedSeries = $event; focusedEvent = null" @clear-focus="focusedEvent = null"/>
      ...
      <MonitorDiagnosticsPanel v-show="tab === 'events' || tab === 'settings'" :candidate-count="totalCandidates" :task-count="totalTasks" :task-id="plan.task_id" @retention-read="emit('retentionRead', $event)" :window="windowRange" :active="tab === 'events' || tab === 'settings'" :mode="tab === 'events' ? 'events' : 'settings'" @locate="locateEvent"/>
