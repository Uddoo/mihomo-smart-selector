...
import type {MonitorActivity, MonitorRow, MonitorRetention, MonitorPlan, MonitorTask, MonitorScheduler} from '../monitoring'
import {taskPath} from './taskState'
import type {MonitorViewState, MonitorDraft} from './taskState'
export interface MonitorTaskViewProps {groups: Group[]; services: ServiceCatalog | null; scanLocked: boolean; task: MonitorTask; tasks: MonitorTask[]; scheduler: MonitorScheduler | null; retention: MonitorRetention | null; viewState: MonitorViewState; totalCandidates: number; totalTasks: number}
export interface MonitorTaskViewEmits {openScan: [group: string, profile: string]; saved: [plan: MonitorPlan]; busy: [value: boolean]; viewState: [id: string, state: MonitorViewState]; retentionRead: [policy: MonitorRetention]}
type Emit = <K extends keyof MonitorTaskViewEmits>(event: K, ...args: MonitorTaskViewEmits[K]) => void
export function useMonitorTaskView(props: MonitorTaskViewProps, emit: Emit) {
  ...
  const {data, failure, lastUpdated, refresh} = useMonitorOverview(windowRange, props.task.plan.task_id, computed(() => tab.value === 'overview' || tab.value === 'details'))
  ...
  const message = ref(''), busy = ref(false), editing = ref(props.viewState.editing), initialized = ref(false)
  ...
  watch(() => props.task.runtime, runtime => {
    if (runtime && data.value && tab.value !== 'overview' && tab.value !== 'details') data.value = {...data.value, current: runtime.current, suspended: runtime.suspended, issue: runtime.issue, observed_at: runtime.observed_at}
  })
  ...
  async function saved(result: MonitorPlan) {
    draft.value = null; emit('saved', result)
  ...
  async function toggle() {
    ...
    try {
      ...
      const result = await api<MonitorPlan>(base, {method: 'PUT', body: JSON.stringify({revision: plan.value.revision, enabled})})
  ...
  async function toggleAuto() {
    ...
    try {
      ...
      const result = await api<MonitorPlan>(base + '/failover', {method: 'PUT', body: JSON.stringify({revision: plan.value.revision, enabled})})
  ...
  async function retest(id: string) {
    ...
    busy.value = true
    try {
      await api(base + '/retest', {method: 'POST', body: JSON.stringify({node_id: id, revision: plan.value.revision})})
      message.value = '复测已排队，受后台预算限制；结果更新健康状态，不替换长期评分样本。'
    } catch (e) { failure.value = e instanceof Error ? e.message : '复测失败' }
    finally { busy.value = false }
  }
  watch(busy, value => emit('busy', value), {flush: 'sync'})
  watch([tab, windowRange, selectedSeries, focusedEvent, editing, draft], () => emit('viewState', props.task.plan.task_id, {tab: tab.value, window: windowRange.value, selectedSeries: selectedSeries.value, focusedEvent: focusedEvent.value, editing: editing.value, draft: draft.value}), {deep: true, flush: 'sync'})
  function cancelEdit() { editing.value = false; draft.value = null }
  const selectedRows = computed(() => data.value?.rows.filter(row => row.series_id === selectedSeries.value) || [])
  return {windowRange, draft, retentionPolicy, data, failure, lastUpdated, refresh, tabs, tab, selectedSeries, focusedEvent, abnormal, unknownCount, openNode, locateEvent, moveTab, windowLabel, message, busy, editing, initialized, plan, current, runLabel, runTone, changeWindow, edit, saved, toggle, toggleAuto, retest, cancelEdit, selectedRows}
}
