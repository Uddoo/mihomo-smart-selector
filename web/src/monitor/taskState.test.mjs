...
const services = {profiles:[{id:'one',probe_count:1},{id:'three',probe_count:3}]}
const task = (id, count, profile='one', enabled=true) => ({scheduled:true,plan:{task_id:id,group:id,profile_id:profile,revision:1,enabled,candidate_limit:30,nodes:Array.from({length:count},(_,i)=>({name:'shared-'+i}))}})
...
test('shared capacity counts each task membership and current-node checks',()=>{
  ...
  assert.equal(capacity.requiredRaw,136080);assert.equal(capacity.requiredHourly,38880)
...
test('paused plans and the edited task are excluded before applying its draft',()=>{
  ...
  assert.deepEqual(monitorWorkload(tasks,services,{taskID:'A',draft}),{candidates:42,groups:2,requests:252,daily:103680})
  assert.equal(monitorWorkload(tasks,services,{taskID:'B',draft:{...draft,enabled:false}}).candidates,6)
...
test('unknown profile does not invent request estimates and stale drafts remain dirty',()=>{
  ...
  const draft={revision:1,group:'A',profile:'one',enabled:true,candidateLimit:30,chosen:plan.nodes.map(n=>n.name),query:''}
  assert.equal(draftChanged(draft,plan),false)
  assert.equal(draftChanged(draft,{...plan,revision:2}),true)
  const a=emptyView(),b=emptyView();a.editing=true;assert.equal(b.editing,false)
