...
async function capture() {
  ...
  try {
    ...
    await page.getByLabel('模式', {exact: true}).selectOption('stable')
    ...
    const evidence = await page.locator('.compare .decision-summary').boundingBox()
    assert(detail && metrics && evidence)
    // An unmodified, tightly cropped portion of the same real rendered page.
    await page.screenshot({path: path.join(shots, 'node-comparison.png'), clip: {x: detail.x, y: detail.y, width: detail.width, height: Math.max(metrics.y + metrics.height, evidence.y + evidence.height) - detail.y + 22}, animations: 'disabled'})
    ...
    const auditPanel = page.locator('section.panel').filter({has: page.getByRole('heading', {name: '节点切换记录'})})
